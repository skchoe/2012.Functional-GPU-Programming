...
(define (edit-dp bg-lst nd-lst i j loloe)
       ...
       (let* ([bzi (zero? i)]
              ...
              [e_i_j
               (cond 
                 ...
                 [bzj i]
                 [else 
                  (let* ([e_im1_j (lol-ref loloe (- i 1) j)]
                         [e_i_jm1 (lol-ref loloe i (- j 1))]
                         [e_im1_jm1 (lol-ref loloe (- i 1) (- j 1))])
                    (min (+ 1 e_im1_j) 
                         (+ 1 e_i_jm1) 
...
(define (edit-distances-dp bg-str nd-str)
  (let* ([bg-lst (string->list bg-str)]
    ...
    (for/fold ([out-lol '()])
      ...
      (for/fold ([out-l out-lol])
        ([j (+ (length nd-lst) 1)])
        (edit-dp bg-lst nd-lst i j out-l)))))
