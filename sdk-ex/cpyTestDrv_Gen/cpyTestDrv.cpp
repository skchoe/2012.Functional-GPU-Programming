...
runTest(int argc, char** argv)
{
    ...
    const char cubin_name [] = "test_kernel.sm_10.cubin";
    ...
    int array_count = 5;
    unsigned int mem_size = array_count * sizeof(float);
    ...
    printf("align of integer = %lu, alignof ptr of integer=%lu\n", __alignof(array_count), __alignof(&array_count));
    printf("sizeof of ptr = %lu, alignof ptr of size_t=%lu\n", sizeof(ptr), __alignof(ptr));
    printf("sizeof of array_count = %lu, alignof array_count of size_t=%lu\n", 
           sizeof(array_count), __alignof(array_count));
    printf("Before align-up of INTEGER, offset(old)=%li\n", offset);
    ALIGN_UP(offset, __alignof(array_count));
    printf("After align-up of INTEGER, offset(new)=%li\n", offset);
    CU_SAFE_CALL(cuParamSeti(cpyTest, offset, array_count));
    offset +=sizeof(array_count);
    printf("Final offset=%li\n", offset);

    // sender h_single
    int h_single_in;
    h_single_in = 256;//atof(argv[1]);
    ALIGN_UP(offset, __alignof(h_single_in));
    printf("After align-up, new offset=%li\n", offset);
    ...
    // receiver d_array_out_len
    CUdeviceptr d_array_out_len;
    cuMemAlloc(&d_array_out_len, sizeof(unsigned int));
    ptr = (void*) (size_t) d_array_out_len;
    printf("ptr=%p, \t &ptr=%p\n", ptr, &ptr);
    ALIGN_UP(offset, __alignof(ptr));
    CU_SAFE_CALL(cuParamSeti(cpyTest, offset, d_array_out_len));
    printf("________d_array_out_len:offset:%lu, size:%lu\n", offset, sizeof(ptr));
    offset += sizeof(ptr);

    // receiver d_single
    CUdeviceptr d_single_out;
    size_t isize = sizeof(int);
    cutilDrvSafeCallNoSync(cuMemAlloc(&d_single_out, isize));
    printf("device pointer = %d\n", d_single_out);

    ptr = (void*)(size_t)d_single_out;
    ALIGN_UP(offset, __alignof(ptr));
    printf("After align-up, new offset=%ld\n", offset);
    //CU_SAFE_CALL(cuParamSeti(cpyTest, offset, d_single_out)); // this stmt works but replaced by below
    CU_SAFE_CALL(cuParamSetv(cpyTest, offset, &ptr, sizeof(ptr)));
...
*/
    ...
    //check if d_array_out has right valuse
    float* h_array_out = (float*)malloc(mem_size);
    cuMemcpyDtoH((void*)h_array_out, d_array_out, mem_size);
    cuMemFree(d_array_out);

    for(int i = 0 ; i < array_count ; i++){
      float value_d = *(h_array_out+i);
      printf ("k->c[%d]=%f\t", i, value_d);
    } printf("\n");
    free(h_array_out);

    size_t* h_single_out = (size_t*) malloc (isize);
    cuMemcpyDtoH((void*)h_single_out, d_single_out, isize);
    printf("hout  = %lu\n", *h_single_out);
    cuMemFree(d_single_out);
    free(h_single_out);

    // length of input arary
    size_t* h_array_out_len = (size_t*)malloc (isize);
    cuMemcpyDtoH((void*) h_array_out_len, d_array_out_len, isize);
    printf("Array outlenth = %ld\n", *h_array_out_len);
    cuMemFree(d_array_out_len);
    free(h_array_out_len);
