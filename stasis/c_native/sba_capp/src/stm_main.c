		...
		int equal_length)
{
  ...
  for(j=0;j<num_var;j++)
    if(0 < num_const[j]) {
      size_t sum_constbytes = sum_const_sizes(constnames[j], num_const[j], var_const_max, uniform_width, equal_length);
  ...
  for(k=0;k<num_var;k++) {
    if(0 < num_const[k]) {
      vars[k] = malloc(sizeof(int*) * num_const[i]);
  ...
  for(l=0;l<num_var;l++) {
    if(constnames[l] != NULL){
      ...
      //what 's output of this?
        int new_len = constraint_length (constnames[l][m], var_const_max, uniform_width, equal_length);
...
void create_constraint_stream_if(int num_var, int var_const_max, int* num_const, byte** constnames, byte** constm,
		int uniform_width,
		int equal_length)
{
  // num_const
                             		...
                             		int equal_length)
{
  ...
  for(i = 0;i < n_var;i++)
  {
    ...
    for(j=0;j<num_const[i];j++)
    {
      ...
      int num_const_var = constraint_length(cn, var_max_size, uniform_width, equal_length);
    ...
    for(j=0;j<num_const[i];j++)
    {
      ...
      int num_cbyte = constraint_length(current_const, var_max_size, uniform_width, equal_length);
...
int main(int argc, char** argv)
{
  ...
  //create_constraint_stream1(NUM_VAR, var_max_size, num_const, constnames, in_constraints, uniform_width, equal_length);
  //create_constraint_stream2(NUM_VAR, var_max_size, num_const, constnames, in_constraints, uniform_width, equal_length);
  create_constraint_stream_if(NUM_VAR, var_max_size, num_const, constnames, in_constraints, uniform_width, equal_length);
  //create_random_constraint_stream(NUM_VAR, NUM_CONST, var_max_size, num_const, constnames, in_constraints, uniform_width, equal_length);
  ...
  // total number of variable: NUM_V	AR
  ...
  solver_constraint_wrapper(ss_in, NUM_VAR, var_max_size, ss_out, uniform_width, equal_length);
