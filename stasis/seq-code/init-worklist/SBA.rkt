...
(define (new-value! var value)
  ...
  ;; Update C for the var
  (foreach-in-constraint var 
                         (lambda (c) (interpret-constraint c value)))
  (printf "E:~a\nC:~a\n" env constraint-env))
...
(define (new-constraint! var constraint)
  ...
  ;; Update E for the var
  (foreach-in-env var 
                  (lambda (value) (interpret-constraint constraint value)))
  (printf "E:~a\nC:~a\n" env constraint-env))
...
(let ([expr (read (open-input-file "test_org.as"))]) ; test_if.as
  (printf "\n\n\nSBA result:~a\n" (SBA expr))
  (printf "ENV:~a\n" env)  
  (printf "CST:~a\n" constraint-env))
;
