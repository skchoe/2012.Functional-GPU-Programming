...
;; proc : (Listof Integer) -> Any
(define (shape-for-each* shp proc . o)
  (let* ([proc-vy (lst2any->vals2any proc)])
    (if (null? o)
        (shape-for-each shp proc-vy)
        (shape-for-each shp proc-vy o))))
...
(define (array-for-each-index* arr proc . o)
  (let* ([proc-vy (lst2any->vals2any proc)])
    (if (null? o)
        (array-for-each-index arr proc-vy)
        (array-for-each-index arr proc-vy o))))
...
(define (tabulate-array* shp proc)
  (let* ([proc-vy (lst2any->vals2any proc)])
        (tabulate-array shp proc-vy)))

(define (tabulate-array!-v shp proc indv)
(let ((arr (make-array shp)))
  (array:arlib:shape-for-each/vector
   shp
...
(define (tabulate-array!-a shp proc inda)
(let ((arr (make-array shp)))
...
;; here, js is {j ...} where j is index of each dimension of arr.
